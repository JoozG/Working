...
if mode == "DNA":
    ...
    with t1:
        ...
        c2.metric("Mean length", f"{df_f['length'].mean():.1f}" if len(df_f) else "–")
        c3.metric("Median length", f"{df_f['length'].median():.1f}" if len(df_f) else "–")
        c4.metric("Avg GC%", f"{df_f['GC%'].mean():.2f}" if len(df_f) else "–")
        ...
        if len(df_f):
            ...
            if picked_id in seqs:
                ...
                with col_a:
                    st.caption("Original (5'→3')")
                    st.code(wrap_seq(orig), language="text")
                    st.caption("Complement (3'→5')")
                    st.code(wrap_seq(comp), language="text")
                with col_b:
                    st.caption("Reverse-complement (5'→3')")
...
# ============= RNA → PROTEIN MODE =============
    ...
    with t1:
        ...
        c1.metric("Sequences", f"{len(df_f):,}".replace(",", " "))
        c2.metric("Mean AA length", f"{df_f['aa_len'].mean():.1f}" if len(df_f) else "–")
        c3.metric("Median AA length", f"{df_f['aa_len'].median():.1f}" if len(df_f) else "–")
        st.dataframe(df_f, use_container_width=True)
...
# ============= PROTEIN MODE =============
    ...
    with t1:
        ...
        c2.metric("Mean length", f"{df_f['length'].mean():.1f}" if len(df_f) else "–")
        c3.metric("Median length", f"{df_f['length'].median():.1f}" if len(df_f) else "–")
        c4.metric("Mean MW (Da)", f"{df_f['MW (Da)'].mean():.1f}" if len(df_f) else "–")
        st.dataframe(df_f, use_container_width=True)
